...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 25-11-2023 a las 00:19:05
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `registrarUsuario` (IN `p_nombres` VARCHAR(100), IN `p_apellidos` VARCHAR(100), IN `p_usuario` VARCHAR(30), IN `p_contraseña` CHAR(100))   BEGIN
    ...
    ELSE
        ...
        VALUES (nuevo_id, p_nombres, p_apellidos, 'C', p_usuario, p_contraseña);
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spPagoViaje` ()   select distinct vianro as viaje, 
(select sum(pago) from pasajeros where vianro=viaje) as total 
from pasajeros order by viaje$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `spTipoViaje` (`numVia` INT)   select distinct (select count(*) from pasajeros where tipo='E' and vianro=numVia) as estudiantes,
(select count(*) from pasajeros where tipo='A' and vianro=numVia) as adultos,
(select count(*) from pasajeros where tipo='N' and vianro=numVia) as niños
from pasajeros where vianro=numVia$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spViajesRuta` ()   select r.RUTCOD,r.RUTNOM Nombre,count(*) Cantidad from ruta r join viaje v on v.RUTCOD=r.RUTCOD
group by r.RUTNOM$$
...
--
-- RELACIONES PARA LA TABLA `chofer`:
--
...
--
-- RELACIONES PARA LA TABLA `comprobante_detalle`:
--   `VIA_NRO`
--       `viaje` -> `VIANRO`
--
...
--
-- RELACIONES PARA LA TABLA `reclamaciones`:
--   `id_usuario`
--       `usuarios` -> `id`
--
...
('USU005', 'Pablo ', 'Villanueva', 'C', 'pablov12', '$2a$10$eXcPruQ8wgfABWOOqNybtOJQ13gJuCxhWXeZ2c/cWAGwg1ZFPlJyO'),
('USU006', 'Lopez', 'Marin', 'C', 'marin12', '$2a$10$zPJQAR2.YdHBeGzzf67tIeEWL/xkdtJKSFPtBKkdeWpdf04vbBc/2');
...
--
ALTER TABLE `comprobante_detalle`
  MODIFY `indice` int(5) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=209;
